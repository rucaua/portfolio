<template>
  <div class="min-h-screen flex flex-col bg-dark text-grey text-md md:text-xl pt-[60px] overflow-x-hidden">
    <animated-back class="z-0 absolute"/>
    <div class="min-h-screen z-10 relative">
      <div
          @click="menuToggle"
          class="fixed w-full h-screen top-[60px] left-0 z-20 transition-all duration-300"
          :class="isMenuOpen ? 'display-block bg-opacity-90 backdrop-blur-sm' : 'hidden bg-opacity-0'"
      ></div>
      <!-- Site Header -->
      <header
          class="fixed top-0 left-0 right-0 h-[60px] z-10 border-b py-4 bg-opacity-90 backdrop-blur-sm px-10 md:px-0">
                  ...
                  <ul class="flex flex-col p-8">
                    <li v-for="linkObj in menu" :key="linkObj.label">
                      <NuxtLink
                          ...
                          @click="menuToggle"
                      >
                        <font-awesome :icon="linkObj.icon" :fixedWidth="true" class="mr-4 text-main"/>
                        {{ linkObj.label }}
                      </NuxtLink>
                    </li>
                  </ul>
      ...
      <footer class="border-t py-4 md:py-14 text-md md:text-xl font-light w-full">
        <div class="container flex flex-col md:flex-row items-center mx-auto">
          <div class="relative p-4 md:p-0 md:pr-20 md:min-w-60 flex flex-row md:flex-col">
            ...
            <div
                class="absolute top-0 bottom-0 right-0 w-[4px] bg-gradient-to-b from-main to-secondary hidden md:block rounded-md"></div>
          </div>
          <div class="p-4 md:p-0 md:pl-20">
            <p>This site was created by Max Tymofeiev with VueJS (Nuxt.js) + Tailwind CSS and deployed via AWS Amplify.
              If
